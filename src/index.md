---
title: Home
description: UK charity funding fertility research to improve diagnosis, treatment and care—working with researchers, clinicians and people with lived experience.
layout: home.njk
permalink: /

# ── Hero ──
hero_title: Fertility Research UK
hero_tagline: ""
hero_lead: >
  We are a new UK-based charity dedicated to funding and
  championing fertility and reproductive health research. We work with
  researchers, clinicians, patients and supporters to accelerate
  discoveries—and turn evidence into better, fairer fertility care.

# ── Our Purpose ──
purpose_intro: >
  Fertility challenges affect people in many ways—yet gaps in evidence and
  uneven access mean too many still face long waits, unclear answers or
  inconsistent support.
purpose_mission: >
  We fund high-quality fertility research and help new knowledge improve care
  in the real world.

callouts:
  - "Around 1 in 7 couples experience infertility."
  - "Many classrooms today include at least one child conceived through fertility treatment, including IVF."
  - "Long-term evidence for people born through assisted reproduction is still growing—especially as the earliest IVF generations become adults."

# ── What We Do cards ──
what_we_do:
  - title: Fund & support research
    description: >
      We raise funds for rigorous studies—from pilots to work that can change clinical practice.
  - title: Bring people together
    description: >
      We connect researchers, clinicians, patients and partners to reduce duplication and speed progress.
  - title: Evidence-based information
    description: >
      We share clear information on what research shows, what is uncertain, and what still needs to be answered.
  - title: Champion better care
    description: >
      We want evidence to lead to earlier diagnosis, safer treatment, clearer guidance and better support.
  - title: Support next-generation researchers
    description: >
      We help build opportunities and visibility for early-career researchers so new ideas are not lost.
  - title: Grow the research community
    description: >
      We encourage more researchers into fertility and reproductive health across disciplines and settings.

---

Behind the headlines, many people go through treatment without success—facing repeated cycles, uncertainty, and strain. Research can improve prediction, personalisation and outcomes.

**That is why Fertility Research UK exists:** to fund research that improves understanding, diagnosis, treatment and care—for everyone affected.
